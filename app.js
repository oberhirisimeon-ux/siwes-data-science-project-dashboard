...
function loadFile(file){const extension=file.name.split('.').pop().toLowerCase();if(extension==='csv'){const reader=new FileReader();reader.onload=()=>setDataset(parseCSV(reader.result),file.name);reader.readAsText(file);return;}if(!['xlsx','xls'].includes(extension)){alert('Please choose a CSV, XLSX, or XLS file.');return;}if(!window.XLSX){alert('The Excel reader did not load. Please refresh and try again.');return;}file.arrayBuffer().then(buffer=>{const workbook=XLSX.read(buffer);const sheetName=workbook.SheetNames[0];const raw=XLSX.utils.sheet_to_json(workbook.Sheets[sheetName],{header:1,defval:''});setDataset(raw,file.name,`Excel sheet: ${sheetName}`);}).catch(()=>alert('This Excel file could not be read. Please try another file.'));}
